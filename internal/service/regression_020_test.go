...
func TestBug020_RetryStopsWhenContextIsCanceled(t *testing.T) {
	...
	started := time.Now()
	err := ingest.Retry(ctx, 3, 200*time.Millisecond, func(context.Context) error { return errors.New("temporary transport") })
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("retry returned %v", err)
	}
	if time.Since(started) > 150*time.Millisecond {
		t.Fatal("retry waited after cancellation")
	}
}
