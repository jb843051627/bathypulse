...
func TestBug026_IngestedPayloadDoesNotShareCallerStorage(t *testing.T) {
	svc := newRegressionService(t)
	addRegressionStation(t, svc, "station-026")
	wave := regressionWave("wave-026", "station-026", time.Now().UTC())
	created, err := svc.IngestWaveform(context.Background(), wave)
	if err != nil {
		t.Fatal(err)
	}
	original := created.Payload[0]
	wave.Payload[0] = original + 1
	if created.Payload[0] != original {
		t.Fatal("returned waveform shares caller payload")
	}
}
