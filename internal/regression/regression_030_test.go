...
func TestBug030_CanceledEventLinkDoesNotCommit(t *testing.T) {
	svc := newRegressionService(t)
	addRegressionStation(t, svc, "station-030")
	event := model.SeismicEvent{ID: "event-030", StationID: "station-030", Kind: "tremor", StartedAt: time.Now().UTC(), EndedAt: time.Now().UTC().Add(time.Second), Magnitude: 3, Confidence: 0.9, State: model.EventOpen, Revision: 1}
	if _, err := svc.OpenEvent(context.Background(), event); err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := svc.LinkEventWaveforms(ctx, event.ID, []string{"wave-030"}); err == nil {
		t.Fatal("canceled event link committed")
	}
	stored, err := svc.GetEvent(context.Background(), event.ID)
	if err != nil || stored.WaveformCount != 0 {
		t.Fatalf("canceled link changed count: %d %v", stored.WaveformCount, err)
	}
}
